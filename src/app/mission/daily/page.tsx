...
export default function DailyMissionPage(){
    ...
    return (
      <main className="container">
        <div className="card" style={{padding:18}}>
          <div className="h2">오늘의 미션을 준비 중…</div>
  ...
  return (
    <main className="container">
      <div className="grid2">
        <div className="card" style={{padding:18}}>
          <div className="badge">오늘의 미션 · 10문제</div>
          <h1 className="h1" style={{marginTop:10}}>문제 {idx+1} / {qs.length}</h1>
          <div className="hr" />
          <div style={{fontSize:26, fontWeight:900}}>{current?.text}</div>
          ...
          <div style={{display:"flex", gap:10, marginTop:14}}>
            <input
              ...
              aria-label="정답 입력"
            />
            <button className="btn btnPrimary" onClick={submit}>제출</button>
          </div>
          ...
          <div style={{marginTop:10}}>
            {answers.length === 0 ? (
              ...
              <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
                {answers.map((a)=>(
                  <span key={a.id} className="badge" style={{color: a.correct ? "var(--good)" : "var(--bad)"}}>
                    {a.correct ? "O" : "X"}
                  </span>
                ))}
              </div>
